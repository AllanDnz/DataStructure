...
#include "ui_listaSEmcadeada.h"
#include <QtGui/qwindowdefs.h>

QString widgetStyles1 = "background: #fff; "
                       "color: #000721;"
                       "height: 500px; "
                       "border-radius: 10px;"
                       "height: 500px;"
...
QString elementoLista1 = "QPushButton {"
                        ...
                        "   font-weight: 400; /* Estilo padrão */"
                        "}";
QString elementoListaEfeito1 = "QPushButton {"
...
listaSEmcadeada::listaSEmcadeada(QWidget *parent, MainWindow *mainWindow) :
    QDialog(parent),
    ui(new Ui::listaSEmcadeada),
    mainWindow(mainWindow) // Inicialize o ponteiro para a MainWindow
{
    ...
    ui->pushButton_2->setStyleSheet(butonsL1);
    ui->pushButton_4->setStyleSheet(butonsL1);
    ui->pushButton_6->setStyleSheet(butonsL1);
    //layout da lista
    ...
    connect(ui->pushButton_6, SIGNAL(clicked(bool)), this, SLOT(buscaElemento()));
...
listaSEmcadeada::~listaSEmcadeada()
{
    delete ui;
}

void listaSEmcadeada::addElemento(){
    ...
    if (lista2.insere(pos, elemento)) {
        ...
        QPushButton* botao = new QPushButton(QString::number(elemento));
        botao->setStyleSheet(elementoLista1);
...
void listaSEmcadeada::changeButtonColor() {
    ...
    if (currentButtonIndex < numButtons && pos >= currentButtonIndex) {
        ...
        if (item) {
            ...
            if (button) {
                ...
                button->setStyleSheet(elementoListaEfeito1);
...
void listaSEmcadeada::restoreButtonColor() {
     int numButtons = ui->horizontalLayout_5->count();
    QString posStr = ui->lineEdit_2->text(); // Suponha que você tenha uma caixa de texto para inserir a posição a ser removida.
    ...
    if (currentChangedButton) {
        currentChangedButton->setStyleSheet(elementoLista1); // supondo que "elementoLista" seja a folha de estilo original
...
void listaSEmcadeada::removerElemento() {
    QString posStr = ui->lineEdit_2->text(); // Suponha que você tenha uma caixa de texto para inserir a posição a ser removida.
...
void listaSEmcadeada::clickBotaoAdd(){
    currentButtonIndex = 0;
    changeButtonColor();
}

void listaSEmcadeada::clickBotaoRemove(){
    currentButtonIndex = 0;
    removeelemento = true;
    changeButtonColor();
}

void listaSEmcadeada::abrirJanelaMenu() {
    if (mainWindow) {
...
void listaSEmcadeada::buscaElemento() {
    QString elementoStr = ui->lineEdit_1->text();
    QString posStr = ui->lineEdit_2->text();
    bool ok;
    int position = 0;
    ...
    if (item) {
        ...
        if (button) {
            ...
            button->setStyleSheet(elementoListaEfeito1); // Altere as cores conforme necessário.
            currentChangedButton = button;
            ...
            QTimer::singleShot(1000, this, [this, position]() {
                currentChangedButton->setStyleSheet(elementoLista1);
            });
        }
    }
}
